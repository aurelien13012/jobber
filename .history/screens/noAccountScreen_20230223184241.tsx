...
const NoAccountScreen = () => {
    ...
    const submitConnection = async (type: string) => {
        const {data, error} = await handleFirstConnection(type, { name, firstName, email, password, companyName })
        console.log("response dans component", error)
        if(error) setErrorMsg(error)
        // if (typeof response === "string") {
        //     setErrorMsg(response)
        // }

        // console.log("response dans component",response.data)
        // if (response.err) setErrorMsg("Oups on a soucis ! on règle ça au plus vite :)")
        // if (response.data) {
